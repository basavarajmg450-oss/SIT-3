...
import { GraduationCap, User, Users, Mail, Lock, Eye, EyeOff, X, Moon, Sun } from 'lucide-react'
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* Form Body */}
            <div className={`transition-colors duration-300 px-6 py-6 ${isDark ? 'bg-slate-900/95' : 'bg-white'}`}>
              {showForgot ? (
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-white' : 'text-slate-700'}`}>Email Address</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-11 pr-4 py-3 rounded-xl border transition-all text-sm outline-none focus:ring-2 focus:ring-white/30 ${isDark
                          ? 'bg-slate-800/80 border-slate-600/50 text-white placeholder-gray-500'
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {isRegister && (
                    <div>
                      <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-white' : 'text-slate-700'}`}>Full Name</label>
                      <input
                        type="text"
                        value={name}
                        onChange={(e) => setName(e.target.value)}
                        placeholder="Your name"
                        className={`w-full px-4 py-3 rounded-xl border transition-all text-sm outline-none focus:ring-2 focus:ring-white/30 ${isDark
                          ? 'bg-slate-800/80 border-slate-600/50 text-white placeholder-gray-500'
                          : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-gray-400'
                          }`}
                      />
                  ...
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-white' : 'text-slate-700'}`}>Password</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="••••••••"
                        className={`w-full pl-11 pr-12 py-3 rounded-xl border transition-all text-sm outline-none focus:ring-2 focus:ring-white/30 ${isDark
                          ? 'bg-slate-800/80 border-slate-600/50 text-white placeholder-gray-500'
                          : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-gray-400'
                          }`}
                        required
